...
- [**4.1.6 DATA USAGE CONTROL**](#416-data-usage-control)
  ...
  - [**4.1.6.4 IDS USAGE CONTROL LANGUAGE**](#4164-ids-usage-control-language)
  - [**4.1.6.5 IDS USAGE CONTROL POLICIES AND POLICY CLASSES**](#4165-ids-usage-control-policies-and-policy-classes)
  - [**4.1.6.6 MANAGEMENT**](#4166-management)
  - [**4.1.6.7 USAGE CONTROL IN A CONNECTOR**](#4167-usage-control-in-a-connector)
  - [**4.1.6.8 APACHE CAMEL INTERCEPTOR (EXAMPLE)**](#4168-apache-camel-interceptor-example)
  - [**4.1.6.9 CONTEXT INFORMATION AND OBLIGATION FULLFILLMENT**](#4169-context-information-and-obligation-fullfillment)
  - [**4.1.6.10 DATA PROVENANCE TRACKING**](#41610-data-provenance-tracking)
    - [**OPERATING PRINCIPLE**](#operating-principle)
    - [**ARCHITECTURE**](#architecture)
    - [**COMMUNICATION**](#communication)
    - [**INTEGRATION WITH DISTRIBUTED USAGE CONTROL**](#integration-with-distributed-usage-control)
  - [**4.1.6.11 DATA PROVENANCE TRACKING ADDRESSED BY THE DIFFERENT LAYERS**](#41611-data-provenance-tracking-addressed-by-the-different-layers)
...
#### **4.1.6.4 IDS USAGE CONTROL LANGUAGE**
...
#### **4.1.6.5 IDS USAGE CONTROL POLICIES AND POLICY CLASSES**

IDS Policy Classes are atomic policy templates that refer to specific restrictions on Data Usage. They are specified in IDS Usage Control Language and define the relevant operands, operators, values and data types for addressing such restrictions. For example, the time-restricted policy class is about restricting the usage of the data to a specific time interval while the corresponding condition only accepts the values that are defined in [xsd:dateTimeStamp](https://www.w3.org/TR/xmlschema11-2/#dateTimeStamp) data type or a policy class may refer to a template that defines a duty to [Delete](https://www.w3.org/TR/odrl-vocab/#term-delete) the data after it is used.  

Furthermore, an IDS Usage Control Policy is an identified policy that consists of one or more instances of the policy classes. For example, the policy X specifies that a Data Consumer shall use the data not later than end of 2024 and only when the location is Germany. Also, the data usage information shall be logged, all the copies of the data shall be deleted not later than a delay period of one month and the Data Provider shall receive a notification when the data is deleted.

An IDS Contract wraps a usage policy with additional information about the involved parties (i.e. the Data Provider and the Data Consumer) and the contract metadata (e.g. issued date). After a successful Contract Negotiation process, the involved parties agree on a set of usage restrictions (i.e. usage policy) as part of their Contract Agreement. Thus, they must assure that these usage restrictions are enforced into their systems (i.e. Connectors). Overall, the [Usage Control in IDS](https://internationaldataspaces.org/data-sovereignty-updated-position-paper-on-data-usage-control-in-the-ids/) provides the possibilities to support the IDS participants to specify and negotiate their usage policies as well as technically enforcing them via various technologies (e.g. [MYDATA](https://www.mydata-control.de/), LUCON, etc.)
...
#### **4.1.6.7 USAGE CONTROL IN A CONNECTOR**
...
#### **4.1.6.8 APACHE CAMEL INTERCEPTOR (EXAMPLE)**
...
Data provenance information is queried at a Privacy Dash- board, which is accessible via a Clearing House. The Privacy Dashboard returns a provenance graph for the unique identifier of data content. There are two options for storing data provenance information:
...
#### **4.1.6.11 DATA PROVENANCE TRACKING ADDRESSED BY THE DIFFERENT LAYERS**
